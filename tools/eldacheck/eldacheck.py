...
def load_checks():
    ...
    from checks import c_syntax, c_links, c_refhealth, c_nodes, c_markers, c_structure, c_text, c_save, c_dead, c_speed, c_dialog, c_chunks, c_world, c_war, c_narr, c_cast, c_lore, c_ui, c_v68ui, c_causality, c_pace, c_textguard, c_skeleton, c_arc, c_report, c_anchor, c_tags, c_lorebook, c_memorybank, c_achievements, c_observe
    return {
        'quick': [
            ('语法检查', c_syntax.run),
            ('死链检查', c_links.run),
        ...
        'full': [
            ...
            ('七锚终局', c_anchor.run),
            ('节点tag通道', c_tags.run),
            ('世界书Lorebook', c_lorebook.run),
            ('记忆注入v3', c_memorybank.run),
            ('成就/图鉴', c_achievements.run),
            ('响应式UI', c_observe.run),
        ],
    }
